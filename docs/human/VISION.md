...
A chatbot replies. This stack is an **operating system**: local secrets, audit, background jobs, copyable universes. Client apps stay outside Helm, the conversational interface.
...
Helm is the conversational interface between a person and the part of the
system in their charge: it turns an expressed intention into a visible,
governed request. The same Helm serves the operator and a customer; what
differs is the jurisdiction it covers and the pilot level the person has
proven. Business tools remain separate applications.
