#!/usr/bin/env bash
# Deploy script for univ-deepharness-dev (DeepSeek Harness / Ollama Cloud gpt-oss:120b)
set -euo pipefail
...
export VAULT_MASTER_KEY="${VAULT_MASTER_KEY:-shaper-deepharness-local-master-key!!}"
export VAULT_TOKEN="${VAULT_TOKEN:-shaper-deepharness-vault-token-local}"
export VAULT_PORT="${VAULT_PORT:-8910}"
export LOGGER_PORT="${LOGGER_PORT:-8920}"
export DEEPSEEK_BRIDGE_PORT="${DEEPSEEK_BRIDGE_PORT:-4350}"
export QUEUE_PORT="${QUEUE_PORT:-8940}"
export MAESTRO_PORT="${MAESTRO_PORT:-8930}"
...
mkdir -p "$SHAPER/data/vault" \
  ...
  "$UNIV/sav/deepseek-ws" "$UNIV/sav/queue" "$UNIV/work"
