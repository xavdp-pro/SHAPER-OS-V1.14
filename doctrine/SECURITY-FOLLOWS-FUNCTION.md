...
| **Podman inside the universe container, one brick one container** | An image carries **exactly the packages its function needs**. Target posture: no SSH, minimal shell, no egress. (Today's DEV deployments still use host networking and alpine shells — locking that down is precisely the hardening pass this doctrine schedules) |
