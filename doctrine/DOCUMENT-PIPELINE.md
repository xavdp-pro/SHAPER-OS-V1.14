...
In the **pipeline Podman's own private MariaDB** (Rule 26), never in an universe-wide or Queue database. The pipeline may consume work through `@shaper/pkg-queue`, but Queue retains its own separate database; typed events connect the functions without sharing storage.
