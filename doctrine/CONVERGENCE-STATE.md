...
| **26** | One mandatory isolated MariaDB per functional Podman | ⚠️ | Operator correction of 22 September 2026: the boundary is each functional Podman, not one shared database per universe. Every function owns its own MariaDB, credentials, storage, backup and restore proof. Static inventory found no fully conformant existing universe: the base template declares five functional Podmans and no MariaDB; Vox declares eight and no MariaDB; Clinic and SuperSIP still use file/CSV paths; PodMesh manager and control journals use SQLite; demo CRM and SaaS prove only an app-owned companion MariaDB, not the rule across every function. Existing working instances remain untouched and may continue as recorded legacy. Compliance becomes mandatory at their next rebuild, replacement, rematerialisation or promotion. **First conformant materialisation, 24 September 2026:** `univ-minimal` (Vault, Logger, Queue, Maestro) runs each unit on its own MariaDB with no network listener, the four-way identity, a `0600` passwd file, DML-only application grants, root by `unix_socket` only, and `pkg-db` in the base; its `proof.sh` reads the Rule 26 gate from outside every unit and one real effect per unit through the root path, and `restore-drill.sh` restores every unit into a throwaway MariaDB (the Vault decrypts a controlled secret and refuses a wrong key). Proven on one DEV instance, across a full container recreation and a universe-container reboot. **Still open:** `univ-base` and `_template` start the same bricks without provisioning their databases, so an image built from this tree refuses to start there until they are converted (the reference universe adds the OpenCode bridge, which needs its own MariaDB too); the catalogue's copy of `pkg-db` is superseded and not yet removed; the manifest's `database` declaration is optional in the schema and not yet a `verify` gate; off-host encrypted recovery points (Rule 16 level 5) are not built. |
