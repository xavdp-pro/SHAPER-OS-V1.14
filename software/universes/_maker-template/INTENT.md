...
| `recipes/` | The frozen recipes, `<kind>-<work>.sh`: one per host kind (`lxd`, `proxmox`, `liblxc`, `nested` — Rule 11's tokens, declared to the maker's recipe runner as `hostKind` — one today; a maker whose machine offers several families selects the prefix from the row's class `shape`, TARGET; reading the kind from the host itself is TARGET; never the bare `lxc`) and per work kind. `lxd-*` ships; `proxmox-*`, `liblxc-*` and `nested-*` are not yet written (`nested`: no terrain in this tree beyond the vzcriu kit; PodMesh, optional, has not stamped one — Rule 11, 16 September 2026), and none ships before terrain (`recipes/README.md`) |
