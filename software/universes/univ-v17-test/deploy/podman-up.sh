...
export VAULT_MASTER_KEY="${VAULT_MASTER_KEY:?Set VAULT_MASTER_KEY in the selected universe env file}"
export SHAPER_IMAGE_TAG="${SHAPER_IMAGE_TAG:?Set SHAPER_IMAGE_TAG to v1.7.1-<git-short-sha>}"
...
if [[ "$BRIDGE_OPENCODE_STUB" != "0" ]]; then
  echo "[podman-up] TEST proof forbids BRIDGE_OPENCODE_STUB=$BRIDGE_OPENCODE_STUB" >&2
  exit 1
fi
if [[ "$MAIL_AGENT_STUB" != "1" ]]; then
  echo "[podman-up] TEST proof requires MAIL_AGENT_STUB=1" >&2
  exit 1
fi
if [[ "$WITH_HELM" != "0" || "$WITH_BRIDGE_CURSOR" != "0" || "$WITH_BRIDGE_AGY" != "0" ]]; then
  echo "[podman-up] manifest declares exactly five Tier-A bricks; optional bricks are forbidden" >&2
  exit 1
fi
...
if [[ ! -f "$SHAPER/data/vault/vault.enc" ]]; then
  ...
  podman run --rm \
    ...
    --entrypoint node \
    localhost/shaper-vault:${SHAPER_IMAGE_TAG} \
...
wait_health() {
  ...
  podman logs --tail 80 "${SLUG}-$name" >&2 || true
  return 1
}
...
podman run -d --name "${SLUG}-logger" --network "$NET" --replace \
  ...
  localhost/shaper-logger:${SHAPER_IMAGE_TAG}

# bootOrder layer 0 must be healthy before bridge-opencode starts.
wait_health vault "http://127.0.0.1:$VAULT_PORT/api/health"
wait_health logger "http://127.0.0.1:$LOGGER_PORT/api/health"
...
podman run -d --name "${SLUG}-bridge-opencode" --network "$NET" --replace \
  ...
  localhost/shaper-bridge-opencode:${SHAPER_IMAGE_TAG}

# bootOrder layer 1 gate.
wait_health bridge-opencode "http://127.0.0.1:$OPENCODE_BRIDGE_PORT/api/health"
...
podman run -d --name "${SLUG}-queue" --network "$NET" --replace \
  ...
  localhost/shaper-queue:${SHAPER_IMAGE_TAG}

# bootOrder layer 2 gate.
wait_health queue "http://127.0.0.1:$QUEUE_PORT/api/health"
...
if [[ "$WITH_BRIDGE_CURSOR" == "1" ]]; then
  ...
  podman run -d --name "${SLUG}-bridge-cursor" --network "$NET" --replace \
    ...
    -v "${WORK_ROOT}:${WORK_ROOT}:Z" \
    localhost/shaper-bridge-cursor:${SHAPER_IMAGE_TAG}
fi
...
if [[ "$WITH_BRIDGE_AGY" == "1" ]]; then
  ...
  podman run -d --name "${SLUG}-bridge-agy" --network "$NET" --replace \
    ...
    -v "${WORK_ROOT}:${WORK_ROOT}:Z" \
    localhost/shaper-bridge-agy:${SHAPER_IMAGE_TAG}
fi
...
podman run -d --name "${SLUG}-maestro" --network "$NET" --replace \
  ...
  -v "$UNIV:/data/univ:Z" \
  localhost/shaper-maestro:${SHAPER_IMAGE_TAG}
...
if [[ "$WITH_HELM" == "1" ]]; then
  ...
  podman run -d --name "${SLUG}-helm" --network "$NET" --replace \
    ...
    localhost/shaper-helm:${SHAPER_IMAGE_TAG}
