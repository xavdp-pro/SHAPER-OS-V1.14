...
# Sourced by proof-effects.sh, definitions only: the real effects of Logger,
# Queue and Maestro,
...
# Persistence of the previous run, per unit. Called by proof-effects.sh when
# a previous run recorded its markers, BEFORE this run writes new ones.
persisted_effects() {
  local job key
...
persisted_logger() {
  ...
  check "logger-persisted" "run's event $id unchanged in logger.events (stored digest)" \
    test -n "$digest" -a "$(rootsql logger logger -e "SELECT digest FROM events WHERE id='$id'")" = "$digest"
}
...
unit_effects() { effects_queue; effects_logger; effects_maestro; }
