#!/usr/bin/env node
/**
 * Sends a prompt to the cursor-agent CLI via bridge, helm-v1 dev session.
 * Usage: npm run cli:dev -- "Add dark mode button"
 */
...
if (!message) {
  console.error('Usage: npm run cli:dev -- "your prompt"');
  process.exit(1);
}
if (!token) {
  console.error('CLI_BRIDGE_TOKEN or ~/.config/cursor-agent-bridge/token required');
  process.exit(1);
}
...
const prefix = 'Read mds/AGENT-DEV.md. ';
