...
| `version-coherence` | Rule 0D — dual intent & topology manifests | Four manifests still named V1.11 in a V1.12 release; then one package said 1.13.23 beside sixteen saying 1.13.2 while verify reported the release named itself once. For a named base (`@shaper/os`) or catalogue (`@shaper/bricks`), the root `package.json` declares the release, independently of the checkout folder. Legacy repositories without those names retain the versioned-folder convention. The check judges the tracked tree (`git ls-files`; disk walk only where git cannot answer): other tracked packages and versioned manifests must match the root exactly, and cross-repo manifest pointers must match its major/minor release. Missing or invalid release metadata and malformed JSON are findings, never a silent skip or a stack trace |
