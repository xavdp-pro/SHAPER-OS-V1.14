...
import { OpencodeBridgeServer } from '../index.js';

describe('bridge-opencode vitals', () => {
  it('publishes evidence and no verdict on GET /api/vitals', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'opencode-vitals-'));
    const bridge = new OpencodeBridgeServer({ stubMode: true, workspaceBase: tmpDir });
