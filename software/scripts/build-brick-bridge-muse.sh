...
IMAGE="${SHAPER_REGISTRY}/shaper/brick-bridge-muse:${SHAPER_IMAGE_TAG}"
podman build \
  --build-arg "SHAPER_BASE_IMAGE=${SHAPER_BASE_IMAGE}" \
  --build-arg "SHAPER_SOURCE_REVISION=${REVISION}" \
  -f bricks/brick-bridge-muse/Containerfile \
  -t "$IMAGE" \
  bricks/brick-bridge-muse
mkdir -p "$ROOT/.release"
podman push --tls-verify="${SHAPER_TLS_VERIFY:-true}" --digestfile "$ROOT/.release/brick-bridge-muse.digest" "$IMAGE"
echo "[build-brick-bridge-muse] published $IMAGE"
