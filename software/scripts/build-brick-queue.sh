...
IMAGE="${SHAPER_REGISTRY}/shaper/brick-queue:${SHAPER_IMAGE_TAG}"
podman build \
  --build-arg "SHAPER_BASE_IMAGE=${SHAPER_BASE_IMAGE}" \
  --build-arg "SHAPER_SOURCE_REVISION=${REVISION}" \
  -f bricks/brick-queue/Containerfile \
  -t "$IMAGE" \
  bricks/brick-queue
...
podman push --tls-verify="${SHAPER_TLS_VERIFY:-true}" --digestfile "$ROOT/.release/brick-queue.digest" "$IMAGE"
echo "[build-brick-queue] published $IMAGE"
