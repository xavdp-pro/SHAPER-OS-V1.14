...
if [[ -n "${VAULT_MASTER_KEY:-}" && "$PRA_ENCRYPTION_KEY" == "$VAULT_MASTER_KEY" ]]; then
  echo "[backup-pra-sync] REFUSED: PRA_ENCRYPTION_KEY must not be the vault master key — a backup encrypted with VAULT_MASTER_KEY hands the vault's key to whoever holds the backup. Generate a separate key: openssl rand -hex 32" >&2
  exit 1
fi
