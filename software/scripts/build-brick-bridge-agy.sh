...
IMAGE="${SHAPER_REGISTRY}/shaper/brick-bridge-agy:${SHAPER_IMAGE_TAG}"
podman build \
  --build-arg "SHAPER_BASE_IMAGE=${SHAPER_BASE_IMAGE}" \
  --build-arg "SHAPER_SOURCE_REVISION=${REVISION}" \
  -f bricks/brick-bridge-agy/Containerfile \
  -t "$IMAGE" \
  bricks/brick-bridge-agy
...
podman push --tls-verify="${SHAPER_TLS_VERIFY:-true}" --digestfile "$ROOT/.release/brick-bridge-agy.digest" "$IMAGE"
echo "[build-brick-bridge-agy] published $IMAGE"
